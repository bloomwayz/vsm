...
let on_hover id =
  ...
  output_json response;
  console_log "Jonna Himdeureotta"
  ...
  try
    while true do
      ...
      let rec read_header () =
        ...
        let () = console_log header in
        if header = "\r" then
          None
        else if String.length header >= 16 && String.sub header 0 16 = "Content-Length: " then
          Some (int_of_string (String.sub header 16 (String.length header - 16)))
        else
          read_header ()
      in
      ...
      | Some content_length ->
        (* Read content *)
        let content = really_input_string stdin content_length in
        console_log ("got content: " ^ content);

        (* Parse JSON request *)
        try
          let request = from_string content in
          let id = request |> Util.member "id" |> Util.to_int in
          ...
          (
            ...
            | _ -> failwith "Other methods unimplemented"
          )

        with
        | Yojson.Json_error msg ->
          console_log ("JSON parsing error: " ^ msg);
    done
  with End_of_file ->
    console_log "Exiting"

let () = Printf.printf "start"; loop ()
